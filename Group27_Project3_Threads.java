...
public class Group27_Project3_Threads implements Runnable{
    ...
    public Group27_Project3_Threads(String strategy1, String strategy2, String strategy3, String strategy4){
        super (); 
    ...
    public void run(){
        ...
        /********************************************************************************************
         START OF YOUR CODE
         ********************************************************************************************/
        	for(roundPlayer=0;roundPlayer<4;roundPlayer++){
            	System.out.println("**********");
        ...
        /********************************************************************************************
         START OF YOUR CODE
         ********************************************************************************************/
	        	if(primary.getPlayer(roundPlayer).getBlood()<1){
	                continue;
	            }else{
	                ArrayList<Integer> handCard = prepare.drawCard(2);
	                prepare.changeHashmap(handCard.get(0),1); 
	                prepare.changeHashmap(handCard.get(1),1); 
	                primary.getPlayer(roundPlayer).getHandCard().add(handCard.get(0));
	                primary.getPlayer(roundPlayer).getHandCard().add(handCard.get(1)); 
	                primary.getPlayer(roundPlayer).setHandCard(primary.getPlayer(roundPlayer).getHandCard());

	                if(roundPlayer==0){
	                	StrategyInterface player0=loadStrategy(inputStrategys[0] ,primary.getPlayer(0),primary.getPlayer(1),primary.getPlayer(2),primary.getPlayer(3));
	                	player0.auto_run(roundPlayer,primary);
	                }else if(roundPlayer ==1){
	                	StrategyInterface player1=loadStrategy(inputStrategys[1] ,primary.getPlayer(1),primary.getPlayer(0),primary.getPlayer(2),primary.getPlayer(3));
	                	player1.auto_run(roundPlayer,primary);
	                }else if(roundPlayer ==2){
	               		StrategyInterface player2=loadStrategy(inputStrategys[2] ,primary.getPlayer(2),primary.getPlayer(0),primary.getPlayer(1),primary.getPlayer(3));
	                	player2.auto_run(roundPlayer,primary);
	                }else if(roundPlayer ==3){
	                	StrategyInterface player3=loadStrategy(inputStrategys[3] ,primary.getPlayer(3),primary.getPlayer(0),primary.getPlayer(1),primary.getPlayer(2));
	                	player3.auto_run(roundPlayer,primary);
	                }
	            }
	        }
         /********************************************************************************************
